...
//      }
  ...
  private fun configureCaptureRequest(config: CameraConfiguration) {
    ...
    val resizedByteBuffer = bitmapToByteBuffer(resizedBitmap)

    // Assuming you have a method createImageFromByteBuffer (not provided, may not exist)
    return createImageFromByteBuffer(resizedByteBuffer, 500, 500)
  }
  ...
  fun byteBufferToBitmap(buffer: ByteBuffer): Bitmap {
    val bytes = ByteArray(buffer.remaining())
    buffer.get(bytes)
  ...
  fun createImageFromByteBuffer(buffer: ByteBuffer, width: Int, height: Int): Image {
    val imageReader = ImageReader.newInstance(width, height, ImageFormat.YUV_420_888, 1)
    val image = imageReader.acquireNextImage()

    if (image != null) {
      try {
        // Copy the data from ByteBuffer to Image
        val planes = image.planes
        val bufferArray = planes[0].buffer.array()
        bufferArray.indices.forEach { bufferArray[it] = buffer[it] }
      } finally {
        // Ensure that you close the ImageReader after usage
        image.close()
      }
    }
